...
// Имя базы задаётся один раз в главном процессе и наследуется рабочими:
// настройка загружается в каждом из них заново, и без этого у каждого была
// бы своя база — а тесты ходят в ту, где живёт поднятый сервер.
const DB_FILE =
  process.env.E2E_DB ?? resolve(TMP, `e2e-${Date.now()}-${process.pid}.db`);
process.env.E2E_DB = DB_FILE;
