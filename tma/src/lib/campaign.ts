...
const PRESETS: Record<string, { title: string; sub: string }> = {
  ...
  waiter: {
    ...
    sub: "Кафе и рестораны рядом ищут людей на завтра. Платят напрямую.",
  },
  barista: {
    title: "Бариста? Смена рядом",
    sub: "Кофейни у дома ищут смену на утро. Платят напрямую, без опыта — тоже берут.",
  },
  student: {
...
export function currentCampaign(): Campaign | null {
  ...
  const preset = key
    ...
    : {
        ...
        sub: "Смены в кафе и ресторанах у дома. Оплата напрямую, без посредников.",
      };
