...
interface Props<T> {
   ...
   *
   *  Получает две ручки, потому что у изнанки свои кнопки: `close` вернуть
   *  карточку лицом, `like` решить, не возвращая. Состоянием переворота
   *  владеет колода, и лезть в него снаружи было бы вторым источником правды. */
  renderBack: (
    item: T,
    controls: { close: () => void; like: () => void },
  ) => React.ReactNode;
