# StaffSwipe — Tinder для подработок в общепите 🇷🇺

Сервис, где соискатели (официант, бариста, повар, посудомой, хостес, бармен) и
работодатели (кафе, бар, ресторан, кофейня) свайпают карточки. При взаимном
лайке — **мэтч → чат → подтверждение смены → PDF-акт** для самозанятого.
Смена, о которой договорились, считается состоявшейся, пока кто-то не нажал
«Смена не состоялась»: молчание больше не означает «работа бесплатно».

## Дизайн и проверенная версия

Кремовая и бордовая палитра, собственные фотографии, галерея профиля,
трёхшаговая публикация с предпросмотром и понятные действия двух ролей.

- [Карта всех 50 экранов: расположение, кнопки, переходы и статус реализации](docs/STAFFSWIPE-UX-50.md).
- [Что проверено и что осталось подключить для запуска](docs/STAFFSWIPE-READINESS.md).
- [Визуальная сверка с примерами](design-qa.md).

| Старт | Лента | Рабочий чат |
| --- | --- | --- |
| ![Старт](docs/design-review/welcome.jpg) | ![Лента](docs/design-review/feed.jpg) | ![Чат](docs/design-review/chat.jpg) |

## Из чего состоит проект

| Компонент | Стек |
| --- | --- |
| **`tma/`** — Telegram Mini App (клиент) | React + TS + Vite + @tma.js/sdk-react |
| **`backend/`** — API + бот | FastAPI + SQLAlchemy + aiogram 3 |

Вход через Telegram (без SMS), нативные кнопки и хаптика.

## 🚀 Запустить у себя (VS Code, самый простой способ)

Нужен только [Node.js LTS](https://nodejs.org) (после установки —
**перезапустить VS Code**). Backend для просмотра НЕ нужен: приложение
работает на демо-данных.

- **Windows** — в терминале VS Code (`Ctrl+~`) выполни: `.\start-app.bat`
  (или просто двойной клик по файлу `start-app.bat` в проводнике).
- **Mac/Linux** — `bash start-app.sh`.

Скрипт сам поставит зависимости и откроет http://localhost:5173/.

Если запускаешь руками, частые ошибки:

| Ошибка в терминале | Причина и лечение |
| --- | --- |
| `npm: не удается найти` / `not recognized` | Node.js не установлен или VS Code открыт до установки → установи LTS с nodejs.org и перезапусти VS Code |
| `выполнение сценариев отключено` (PowerShell) | Политика PowerShell блокирует `npm.ps1` → запускай `.\start-app.bat` (он идёт через cmd) или выполни `Set-ExecutionPolicy -Scope CurrentUser RemoteSigned` |
| `ENOENT: no such file... package.json` | Проверьте папку: запуск доступен из корня репозитория (`npm run dev`) или из `tma` |
| Порт занят / страница пустая | Vite напишет другой адрес (`:5174`) — открой его; страницу открывать в Chrome |

```bash
# То же самое руками:
cd tma && npm install && npm run dev

# Backend (не обязателен для просмотра):
cd backend && pip install -r requirements.txt && uvicorn app.main:app --reload
```

Проверка всего проекта одной командой: `bash scripts/verify.sh`.

## Что реализовано

- Онбординг, выбор роли, профиль (роли, медкнижка, самозанятость, фото).
- **Свайп-лента** и альтернативный список; фильтры (роль, ставка, дата, без
  медкнижки/опыта, только проверенные, сортировка).
- Мэтч → **чат** (REST + WebSocket) → подтверждение смены обеими сторонами.
- **Закрытие смены молчанием**: договорились — значит смена состоялась, пока
  кто-то не нажал «Смена не состоялась». Через 12 часов после окончания она
  закрывается сама и начисляется комиссия; утром обе стороны получают
  предупреждение. Отметка кодом заведения не обязательна — это доказательство
  работника в споре.
- Отмена и перенос смены (только до начала), уточнение фактических часов
  (только в первые сутки), споры с разбором оператором.
- **Планировщик** (`scheduler`): напоминания 9:00, вопрос про вчерашние смены
  9:30, закрытие и комиссия 14:00, смены без людей 19:00, сверка платежей 4:30.
- **PDF-акт** выполненных работ для самозанятого.
- Монетизация: **комиссия 10% с закрытой смены**, списывается с баланса
  заведения; баланс пополняется картой через ЮKassa.
- Рост: рефералы (счётчик «кто кого привёл»), сохранённые поиски с алертами
  бота, шеринг смены.
- Тёмная тема, аналитика воронки, согласие 18+ / 152-ФЗ.

### Монетизация

| Источник | Что | Платёж |
| --- | --- | --- |
| Комиссия сервиса | 10% от оплаты **закрытой** смены | списание с баланса |
| Пополнение баланса | аванс заведения, из которого идут списания | ЮKassa |

Больше платить не за что: публикация смен, отклики, чат и акт для
самозанятого — бесплатны и без лимитов. Ни подписок, ни платного поднятия
в ленте, ни внутренних «валют»: место в ленте не продаётся, а деньги сервис
берёт только когда человек реально вышел на смену. Подробно —
**[`docs/COMMISSION.md`](docs/COMMISSION.md)**.

## ⚠️ Прод-конфиг: вставить ключи и запустить

Прод настраивается ОДНИМ корневым `.env` — шаблон **`.env.example`** в корне
репозитория (его читает `docker-compose.prod.yml`). Файлы
`backend/.env.example` и `tma/.env.example` нужны только для локального
запуска без docker. В прод-режиме (`DEV_MODE=false`) приложение
**не стартует** с дефолтными секретами — это защита от случайного небезопасного
деплоя.

| Переменная | Зачем |
| --- | --- |
| `JWT_SECRET` | подпись токенов (обязательно сменить) |
| `INTERNAL_API_SECRET` | защита `/billing/fulfill` и вебхука (бот/ЮKassa) |
| `ALLOW_INSECURE_TELEGRAM_AUTH=false` | запретить вход без подписи initData |
| `TELEGRAM_BOT_TOKEN` | валидация initData, уведомления в Telegram |
| `ALLOWED_ORIGINS` | CORS: URL Mini App вместо `*` |
| `YOOKASSA_SHOP_ID` / `YOOKASSA_SECRET_KEY` | приём рублёвых платежей |
| `DADATA_TOKEN` | верификация ИНН/ОГРН и подсказки адресов |
| `S3_*` | хранилище фото (Yandex Object Storage / S3) |

Безопасность (в коде): доступ к чату/акту/подтверждению только участникам мэтча;
лента кандидатов — только работодателю, без ИНН; акт — только после подтверждения
смены; идемпотентные свайпы и уникальные ключи в БД (нет двойных списаний/мэтчей);
рейт-лимит на запись и на SMS-коды (срок жизни кода 10 мин); строгая валидация
входных данных; неблокирующие уведомления.

🚀 Запуск сегодня по шагам (бот и домен уже есть) — **[`docs/ПУСК.md`](docs/ПУСК.md)**.
🧭 С чего начать, если пока ничего нет — **[`docs/START-TODAY.md`](docs/START-TODAY.md)**.
📦 Запуск с нуля, по шагам и по кнопкам — **[`docs/ПУСК.md`](docs/%D0%9F%D0%A3%D0%A1%D0%9A.md)**.
Технические подробности деплоя — **[`docs/DEPLOY.md`](docs/DEPLOY.md)**.
📷 Хранилище фото (S3, Россия — 152-ФЗ) — **[`docs/ФОТО.md`](docs/%D0%A4%D0%9E%D0%A2%D0%9E.md)**.

## Модель данных

`users`, `employers`, `vacancies`, `swipes`, `matches`, `messages`,
`entitlements`, `purchases`, `commissions`, `wallet_txns`, `phone_codes`,
`referrals`, `reviews`, `events`, `saved_searches`, `favorites`, `reports`.
См. `backend/app/models.py`.
