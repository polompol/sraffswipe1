...
живом сервере ДО правки:
    ...
    INFO: 127.0.0.1:35358 - "WebSocket /ws/chat/abc?token=<здесь стоял
    настоящий токен целиком>" 403

Сам токен в примере не приводится намеренно: строка, похожая на настоящий
ключ, попадает в поиск секретов по истории и роняет сборку — проверено, она и
упала. Пояснению это ничего не стоит, а сканер остаётся строгим.
...
# Точки в значении важны: фильтр вырезает всё до пробела, и разделители
# надо проверить. А на настоящий ключ строка похожа быть не должна.
SECRET = "testovyj.TOKEN-DLYA-PROVERKI.podpis"
