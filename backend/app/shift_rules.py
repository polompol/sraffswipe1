...
def shift_is_over(db: Session, m: Match) -> bool:
    ...
    # Битую дату считаем «смена ЕЩЁ НЕ закончилась», то есть закрываем ворота.
    #
    # Стояло обратное — `return True` с оговоркой «не мешаем людям работать».
    # Звучит человечно, а на деле это фраза-ключ ко всему, что ворота стерегут:
    # заведение отмечает работнику неявку за неделю ДО смены; пара аккаунтов
    # набивает рейтинг за минуты; акт «услуги оказаны полностью и в срок»
    # выписывается на работу, которой не было; расчёт закрывает смену и берёт
    # комиссию за неотработанное. Достаточно одной вакансии с неразобранной
    # датой — и всё это открывается разом.
    #
    # «Смены нет» и «дату не разобрать» — разные случаи, поэтому и ответы
    # разные. В первом мы ЗНАЕМ, что защищать нечего. Во втором мы не знаем
    # ничего — а на незнании деньги не двигают.
    try:
