...
def mark_not_held(
    ...
    # Показания расходятся — решает оператор, а не кнопка.
    #
    # Проверялся только флаг ЧУЖОЙ стороны, и из-за этого оставался открытый
    # канал мимо комиссии. Работник называет код (это доказательство сервиса,
    # что он был на месте: код говорят при встрече), а потом сам жмёт «Смена
    # не состоялась». Заведение просто не нажимает «Человек пришёл». Спор не
    # открывался, смена уходила в expired, комиссия не начислялась никогда, и
    # оператор об этом не узнавал. Двумя тапами по сговору — ровно та же
    # дыра, что и «молчание = не платить», только с другой стороны.
    #
    # Теперь отметка кодом весит одинаково против кого угодно, включая того,
    # кто её поставил: запись, которая противоречит сама себе, к оператору.
    #
    # Законный путь работника не задет: «пришла, а заведение закрыто» — это
    # случай, когда кодом он НЕ отмечался (кода ему никто не назвал), и такая
    # смена по-прежнему закрывается без денег и без неявки.
    contradicted = (
        m.seeker_checked_in or (is_seeker and m.employer_checked_in)
    )
    if contradicted:
        open_dispute(
            db, m,
