...
### Прогон на настоящем PostgreSQL

Обычный прогон идёт на SQLite: быстро и без внешних зависимостей. Но работает
сервис на PostgreSQL, а тот прощает меньше — строгие типы, внешние ключи,
уникальность при NULL. Чтобы эти расхождения не всплыли уже на сервере, тот же
набор тестов можно прогнать на настоящей базе:

```bash
# База поднята отдельно; схему накатят миграции, данные чистятся между тестами
STAFFSWIPE_TEST_DB="postgresql+psycopg://user:pass@localhost:5432/staffswipe_test" \
    pytest -q
```

Драйвер тот же, что в проде (`+psycopg`, это psycopg3), — иначе проверяется не
то соединение, с которым сервис живёт.
