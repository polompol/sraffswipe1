...
def collect() -> dict[str, list[tuple[str, int, str]]]:
    ...
    for p in sorted((ROOT / "backend" / "app").rglob("*.py")):
        ...
        consts = dict(
            re.findall(
                # Пробелы здесь ТОЛЬКО [ \t], а не \s.
                #
                # \s включает перевод строки, и тогда «\s*\n» одной строки и
                # «\s*» следующей делят одни и те же символы: у движка
                # появляется экспоненциально много способов разбить один и тот
                # же кусок текста, и на неподходящем конце он их перебирает
                # все. Замерено: 22 пары строк — 1,2 с, каждые следующие
                # четыре умножают время примерно на шестнадцать.
                #
                # Нашёл это не я, а CodeQL. Я перед тем проверил выражение
                # руками, подобрал слабый вход, ничего не увидел и решил, что
                # чисто. Замер без правильного входа доказывает только то, что
                # вход был неправильный.
                r'^(_[A-Z][A-Z0-9_]*)[ \t]*=[ \t]*\([ \t]*\n'
                r'((?:[ \t]*"(?:[^"\\]|\\.)*"[ \t]*\n)+)[ \t]*\)',
                src, re.M,
            )
